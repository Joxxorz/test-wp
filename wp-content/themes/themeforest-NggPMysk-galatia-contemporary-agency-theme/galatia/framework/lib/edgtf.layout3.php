...
/*
   Class: GalatiaEdgeClassMultipleImages
   A class that initializes Edge Multiple Images
*/

class GalatiaEdgeClassMultipleImages implements iGalatiaEdgeInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$galatia_edge_global_Framework->edgtMetaBoxes->addOption( $this->name, "" );
	}
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="edgtf-gallery-uploader">
                                <a class="edgtf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'galatia' ); ?></a>
                                <a class="edgtf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'galatia' ); ?></a>
                            </div>
...
class GalatiaEdgeClassTwitterFramework implements iGalatiaEdgeInterfaceRender {
	public function render( $factory ) {
		$twitterApi = GalatiaTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'galatia' );
			} else {
				$message = $responseObj->message;
			}
		}

		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'galatia' ) : esc_html__( 'Connect with Twitter', 'galatia' );
		?>
		<?php if ( $message !== '' ) { ?>
        ...
        <div class="edgtf-page-form-section" id="edgtf_enable_social_share">
            <div class="edgtf-field-desc">
                <h4><?php esc_html_e( 'Connect with Twitter', 'galatia' ); ?></h4>
                <p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'galatia' ); ?></p>
            </div>
            <div class="edgtf-section-content">
                <div class="container-fluid">
                    <div class="row">
...
class GalatiaEdgeClassInstagramFramework implements iGalatiaEdgeInterfaceRender {
	public function render( $factory ) {
		$instagram_api = GalatiaInstagramApi::getInstance();
		$message       = '';

		//if code wasn't saved to database
		if ( ! get_option( 'galatia_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'galatia' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'galatia' ) : esc_html__( 'Connect with Instagram', 'galatia' );
		?>
		<?php if ( $message !== '' ) { ?>
            <div class="alert alert-success">
                <span><?php echo esc_html( $message ); ?></span>
        ...
        <div class="edgtf-page-form-section" id="edgtf_enable_social_share">
            <div class="edgtf-field-desc">
                <h4><?php esc_html_e( 'Connect with Instagram', 'galatia' ); ?></h4>
                <p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'galatia' ); ?></p>
            </div>
            <div class="edgtf-section-content">
                <div class="container-fluid">
                    <div class="row">
...
class GalatiaEdgeClassRepeater implements iGalatiaEdgeInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'galatia' );
			...
			<?php if ( $this->table_layout ) { ?>
                <div class="edgtf-repeater-table-heading">
                    <div class="edgtf-repeater-fields-holder">
                        <div class="edgtf-repeater-table-cell edgtf-repeater-sort"><?php esc_html_e( 'Order', 'galatia' ) ?></div>
                        ...
                        <div class="edgtf-repeater-table-cell edgtf-repeater-remove"><?php esc_html_e( 'Remove', 'galatia' ) ?></div>
                    </div>
                </div>
			<?php } ?>
                                                                            ...
                                                                            <div class="edgtf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
                                                                                <div class="edgtf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = galatia_edge_return_repeater_dependency_options_array( array(
																							'field'        => $field,
...
class GalatiaEdgeClassFieldAddress extends GalatiaEdgeClassFieldType {
                    ...
                    <div class="row">
						...
						<?php endif; ?>
							...
							if ( empty( $google_maps_api_key ) ) { ?>
                                <p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Galatia Options', 'galatia' ); ?></p>
							<?php } ?>
                            <a class="edgtf-reset-marker edgtf-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'galatia' ); ?></a>
...
class GalatiaEdgeClassFieldIcon extends GalatiaEdgeClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = galatia_edge_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = galatia_edge_icon_collections()->getIconCollectionsKeys();
		?>
            ...
            <div class="edgtf-section-content">
                <div class="container-fluid">
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = galatia_edge_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						$field_class = ! empty( $rvalue ) && $rvalue['icon_pack'] == $icons_collection ? 'edgtf-show-field' : 'edgtf-hide-field';
						?>
                        <div class="row edgtf-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                            <div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" class="<?php echo esc_attr( $select2 ) ?> form-control edgtf-form-element">
									<?php
									$icons       = galatia_edge_icon_collections()->getIconCollection( $icons_collection );
									$active_icon = $rvalue[ $icons_param ];
									foreach ( $icons->icons as $option => $key ) { ?>
                                        <option value="<?php echo esc_attr( $key ); ?>" <?php if ( $key == $active_icon ) {
											echo 'selected';
